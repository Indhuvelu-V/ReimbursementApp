...
export class ExpenseCategories implements OnInit {
  ...
  loadCategories() {
    ...
    this.categoryService.apiGetAllCategories().subscribe({
      ...
      error: (err)  => {
        this.toast.showError('Failed to load categories. Please refresh.');
        this.loader.hide();
      }
    });
  }
  ...
  updateCategory() {
    ...
    if (this.newMaxLimit <= 0) {
      this.toast.showWarning('Max limit must be greater than zero.');
      return;
    }

    const request: CreateExpenseCategoryRequestDto = {
      categoryName: this.selectedCategory.categoryName,  // enum key
      maxLimit:     this.newMaxLimit
    };

    this.loader.show();
    // Fix: use correct PUT /api/ExpenseCategory endpoint
    this.categoryService.apiUpdateCategory(request).subscribe({
      ...
      error: (err) => {
        this.toast.showError(
          err?.error?.message || 'Failed to update category. Please check the values.'
        );
        this.loader.hide();
      }
    });
